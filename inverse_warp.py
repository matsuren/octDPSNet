...
def pixel2cam(depth, intrinsics_inv):
    ...
    if (pixel_coords is None) or pixel_coords.size(2) < h:
        set_id_grid(depth)
    current_pixel_coords = pixel_coords[:, :, :h, :w].expand(b, 3, h, w).contiguous().view(b, 3, -1).to(
        device)  # [B, 3, H*W]
...
def inverse_warp(feat, depth, pose, intrinsics, intrinsics_inv, padding_mode='zeros'):
    ...
    projected_feat = torch.nn.functional.grid_sample(feat, src_pixel_coords,
